    ...
    function layout_shows($start, $end, $message, $minrating=1, $null=1)
    {
        ...
        if ($shows or $null){
            ...
            foreach($client->getShows($start, $end, $minrating, $null) as $showinfo){
                ...
                echo '<a href="set.php?show='.urlencode($showinfo["Show Name"]).'&rating=0" onclick="return setRating(0, \''.str_replace("'" , "\'", $showinfo["Show Name"]).'\')"><img src="x.png" /></a>';
                for($a=0 ; $a < $showinfo["Rating"] ; $a = $a + 1){
                    echo '<a href="set.php?show='.urlencode($showinfo["Show Name"]).'&rating='.($a+1).'" onclick="return setRating('.($a+1).', \''.str_replace("'" , "\'", $showinfo["Show Name"]).'\')"><img src="black.png" /></a>';
                }
                for(;$a < 5; $a = $a + 1){
                    echo '<a href="set.php?show='.urlencode($showinfo["Show Name"]).'&rating='.($a+1).'" onclick="return setRating('.($a+1).', \''.str_replace("'" , "\'", $showinfo["Show Name"]).'\')"><img src="white.png" /></a>';
                }
                echo "</td>\n";
                echo "            </tr>\n";
            }
